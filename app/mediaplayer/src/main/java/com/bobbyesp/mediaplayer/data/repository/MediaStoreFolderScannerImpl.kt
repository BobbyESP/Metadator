...
class MediaStoreFolderScannerImpl : MediaStoreFolderScanner {
    override suspend fun scanCustomFolder(path: String): List<MusicTrack> =
        withContext(Dispatchers.IO) {
            ...
            return@withContext folder.walkTopDown()
                .filter { it.isFile && it.extension.lowercase() in allowedExtensions }.map { file ->
                    MusicTrack(
    ...
    override fun forceMediaStoreFolderScanning(context: Context, path: String) {
        val file = File(path)
        if (!file.exists() || !file.isDirectory) throw IllegalArgumentException("An invalid path was provided to be scanned by MediaStore")
