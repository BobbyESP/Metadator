...
class SpotifyServiceImpl : SpotifyService, KoinComponent {
   ...
   *
   * This function returns a [SpotifyAppApi] instance, allowing interaction with the Spotify API. It
   * ensures that the API is built before returning it. If the API has not been built yet, it will
   * call the [buildApi] function to initialize it.
   *
   ...
   *
   * This function fetches the Spotify access token, ensuring it's available before returning it. If
   * the token is null, it attempts to build/refresh the token via the `buildApi()` method.
   *
  ...
  /**
   * This method is responsible for building the Spotify API. It first checks if the application is
   * in debug mode, and if so, logs the client ID and secret. Then, it attempts to build the Spotify
   * API with the provided client ID and secret. If the API is successfully built, it retrieves the
   * token from the API and stores it.
   *
