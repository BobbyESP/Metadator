...
internal object MenuPosition {
  ...
  /**
   * An interface to calculate the horizontal position of a menu with respect to its anchor, window,
   * and layout direction. The returned x-coordinate is relative to the window.
   *
   * @see PopupPositionProvider
   */
  @Stable
   ...
   * anchor.
   *
   * The given [offset] is [LayoutDirection]-aware. It will be added to the resulting x position for
   * [LayoutDirection.Ltr] and subtracted for [LayoutDirection.Rtl].
   */
...
internal object AnchorAlignmentOffsetPosition {
  /**
   * A [MenuPosition.Horizontal] which horizontally aligns the given [menuAlignment] with the given
   * [anchorAlignment].
   *
   * The given [offset] is [LayoutDirection]-aware. It will be added to the resulting x position for
   * [LayoutDirection.Ltr] and subtracted for [LayoutDirection.Rtl].
...
internal object WindowAlignmentMarginPosition {
   ...
   *
   * The resulting x position will be coerced so that the menu remains within the area inside the
   * given [margin] from the left and right edges of the window. If this is not possible, i.e., the
   * menu is too wide, then it is centered horizontally instead.
   */
  @Immutable
  data class Horizontal(
   ...
   *
   * The resulting y position will be coerced so that the menu remains within the area inside the
   * given [margin] from the top and bottom edges of the window. If this is not possible, i.e., the
   * menu is too tall, then it is centered vertically instead.
   */
  @Immutable
  data class Vertical(
...
internal data class DropdownMenuPositionProvider(
  ...
  override fun calculatePosition(
    ...
    val x =
        xCandidates.fastFirstOrNull { it >= 0 && it + popupContentSize.width <= windowSize.width }
            ?: xCandidates.last()
