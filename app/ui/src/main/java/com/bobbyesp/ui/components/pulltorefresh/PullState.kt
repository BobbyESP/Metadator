        ...
        override fun onPostScroll(
          ...
          when {
            ...
            available.y > 0 && source == NestedScrollSource.UserInput -> {
              // 1. User is dragging
              // 2. Scrollable container reached the top (OR max drag reached and neither scroll
              ...
              // -> Start drag. Because next frame offsetY will be > 0f, onPreScroll will take over
              // from here
              isDragging = true
              scope.launch { _offsetY.snapTo((offsetY + available.y).coerceIn(0f, heightMax)) }
            }
          }
